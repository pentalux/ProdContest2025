...
def main():
    ...
    # Всегда обновляем подписку при загрузке страницы
    user_data = update_user_subscription(user_data)
    user_sessions[user_id] = user_data  # Обновляем в сессии
...
def admin_panel():
    ...
    # Получаем всех пользователей из базы данных (подписки обновляются автоматически в get_all_users_from_db)
...
def admin_edit_user(user_unique_id):
    ...
    # Определяем подписку пользователя
...
@app.route('/update-subscription-description', methods=['POST'])
def update_subscription_description():
    """Обновляет описание подписки"""
    user_id = request.form.get('user_id')
    subscription_id = int(request.form.get('subscription_id'))
    new_description = request.form.get('description')
    ...
    success = subscription_manager.update_subscription_description(subscription_id, new_description)
...
@app.route('/admin-empty')
def admin_empty():
    """Пустая страница для админов"""
    user_id = request.args.get('user_id')
    if not user_id or user_id not in user_sessions:
        return redirect(url_for('index'))
    ...
    return render_template('admin_empty.html', user=user_data)
...
def check_auth(auth_id):
    print(f"Checking auth for: {auth_id}")
    session = auth_storage.get_session(auth_id)
    
    if not session:
        print(f"Session not found: {auth_id}")
    ...
    if session['status'] == 'completed':
        ...
        # Подписка уже обновлена в complete_session
        ...
        # Всех пользователей (включая админов) перекидываем на главную страницу
